...
trait FunSets extends FunSetsInterface:
   ...
   */
  def filter(s: FunSet, p: Int => Boolean): FunSet = (x:Int) => contains(s, x) && contains(p, x)
  ...
  def forall(s: FunSet, p: Int => Boolean): Boolean ={
    def iter(a: Int): Boolean ={
      if (a > bound) then
        true
      else if (contains(s,a) && !contains(p,a)) then
        false
      else
        iter(a+1)
    }
   ...
   */
  def map(s: FunSet, f: Int => Int): FunSet = (x:Int) => exists(s, y => f(y) == x )
